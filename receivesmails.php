<?php
	...
	if (isset($_POST['comment'])){
		...
		mysql_query("INSERT INTO reply (messageid, receiver, recipient, contentreply, date)
		VALUES ('$msgid','','$recipient','$contentreply',NOW())")or die(mysql_error());
	}
?>
		...
		<div class="art-post-inner art-article">
			...
			<div class="art-block-body">
						...
						<div class="cleared"></div>
							...
							<?php 			
									$member_id = $_SESSION['member_id'];
									$post = mysql_query("SELECT * FROM messages WHERE receiver = '$member_id' ORDER by datetime DESC")or die(mysql_error());
									while($row = mysql_fetch_array($post)){
										...
										$allcomm = mysql_query("SELECT * FROM reply WHERE messageid = '$iyaid'")or die(mysql_error());
											$counters = 0;
										...
										echo "<input type='hidden' value='".$row['message_id']."' name='cantseeme'/>
											...
											".$row['content']."<br /><br /><strong>Last</strong> ".$row['datetime']."<hr /> <a href='replies.php?id=".$row['message_id']."' rel='facebox' style='text-decoration:none;'>Replies(".$allcount.")</a>
											<div style='float:right;'><a href='deletemessage.php?message_id=".$row['message_id']."' style='text-decoration:none;'>Delete</a></div>
											<hr />
				...
				<ul>
				 ...
				 <?php
						...
						while($row = mysql_fetch_array($post)){
							echo '
							<li>
								<p align="center"><a href="userprofiletest.php?id='.$row['member_id'].'" style="text-decoration:none;"><img src="image/members/'.$row['photo'].'" alt="" height="70" width="70" border="0" class="left_bt" />
